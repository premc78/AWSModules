...
provider "aws" {
  ...
  assume_role {
    ...
    session_name = "AWSControlTowerExecution" 
  }
}
...
resource "aws_vpc_peering_connection" "this" {
  ...
  tags          = merge(
    var.tags, 
    map("Side", local.same_acount_and_region ? "Both" : "Requester"),
    map("Name", "vpc-vpc-peering")
    )
}
...
resource "aws_route" "this_routes_region" {
  ...
  depends_on = [data.aws_route_tables.this_vpc_rts]
}
...
resource "aws_route" "peer_routes_region" {
  ...
  depends_on = [data.aws_route_tables.peer_vpc_rts]
}