...
data "aws_iam_policy_document" "eks-workers-allow-assume-policy" {
  count = length(var.roles_workers_can_assume) > 0 ? 1 : 0
  statement {
    sid = "1"
    ...
    resources = var.roles_workers_can_assume
  }
}
resource "aws_iam_policy" "eks-workers-allow-assume-policy" {
  count       = length(var.roles_workers_can_assume) > 0 ? 1 : 0
  name        = "${aws_iam_role.workers[0].name}-allow-assume"
  description = "Allow workers to call assume"
  policy      = element(data.aws_iam_policy_document.eks-workers-allow-assume-policy.*.json, 0)
}

resource "aws_iam_role_policy_attachment" "eks-workers-allow-assume" {
  count      = length(var.roles_workers_can_assume) > 0 ? 1 : 0
  role       = aws_iam_role.workers[0].name
  policy_arn = element(aws_iam_policy.eks-workers-allow-assume-policy.*.arn, 0)
}